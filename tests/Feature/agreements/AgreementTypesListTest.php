...
class AgreementTypesListTest extends TestCase
{
    ...
    {
        $response = $this->get(route('agrTypes'));
    ...
    public function testVisitCompaniesListAsNewSuperuser(): void
    {
        ...
        $response = $this->actingAs($user)->get(route('agrTypes'));
    ...
    public function testVisitCompaniesListAsSuperuser(): void
    {
        ...
        $response = $this->actingAs($user)->get('agrTypes');

        $response->assertStatus(200)
        ->assertSee('Виды договоров')
        ->assertSee('Наименование')
        ->assertSee('Покупка/продажа')
        ->assertSee('Сфера деятельности')
        ->assertSee('Кредит/займ')
        ;
    }
