<table class="table">
    ...
    @forelse($deposits as $deposit)
        <tr @if($deposit->real_date_close) class="text-secondary text-decoration-line-through" @endif>
            ...
            <td>
                @if(Gate::allows('s-vehicle'))
                    <a href="{{route('admin.vehicleSummary',['vehicle'=>$deposit->vehicle])}}"> &#9776;Карточка </a></td>
                @endif    
            <td>
                @if(Gate::allows('e-agreement'))
                <a href="{{route('admin.editDeposit', ['deposit'=>$deposit])}}">
                    &#9998;Изменить </a>
                @endif    
            </td>
            <td>
                @if(Gate::allows('e-agreement'))
                <a href="{{route('admin.deleteDeposit', ['deposit'=>$deposit])}}"
