...
@section('content')
    <h3>Добавить документы для задачи: {{$task->subject}}</h3>

    <form id="multipleDocumentsForm" method="POST" enctype="multipart/form-data" 
    action="{{ route('addTaskManyDocuments', ['task' => $task->id]) }}">
        @csrf
        ...
        <a class="btn btn-secondary mt-3" href="{{ route('taskCard', ['task' => $task->id, 'page' => 'documents']) }}">Отмена</a>
    </form>
@endsection
